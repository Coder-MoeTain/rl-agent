...
class PPO_PER(PPO):
    def __init__(
        self,
        *args,
        per_capacity: int = 4096,
        per_alpha: float = 0.6,
        per_beta_start: float = 0.4,
        per_beta_frames: int = 100000,
        per_batch_size: int = 64,
        **kwargs,
    ):
    ...
    def collect_rollouts(
        self, env, callback, rollout_buffer: RolloutBuffer, n_rollout_steps: int
        ...
        for epoch in range(self.n_epochs):
            ...
            if use_per:
                try:
                    ...
                    logger.warning(
                        f"Error in PER training step: {e}, falling back to standard training"
                    )
                    use_per = False
